...
struct CoachGuideView: View {
    /// Curriculum editing and media upload are head-coach-only, enforced by the
    /// database. This sheet auto-presents on a coach's first visit, so leaving
    /// those two sections in would hand a regular coach — including Apple's
    /// reviewer, on their very first screen — instructions for a control that
    /// isn't there.
